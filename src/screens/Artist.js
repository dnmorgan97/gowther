...
class Artist extends React.Component {
  ...
  async getUser(user_id) {
    ...
    const refreshToken = await this.state.spotifyApi.getUserData('refreshToken');

    const response = await fetch('https://accounts.spotify.com/v1/users/' + user_id, {
      method: 'GET',
      headers: {
    ...
    this.state.artist = responseJson;
  ...
  componentDidMount() {
    ...
    const { user_id } = screenProps;
    this.state.artist = user_id;
